...
{
    // Keep data_len / payload intact for the consumer; reset happens in frame_return_empty.
...
frame_t *frame_try_get_filled(void)
{
    frame_t *this_fb;
    if (filled_fb_queue == NULL) {
        return NULL;
    }
    if (xQueueReceive(filled_fb_queue, &this_fb, 0) == pdPASS) {
        return this_fb;
    }
    return NULL;
}
...
{
    if (empty_fb_queue == NULL && filled_fb_queue == NULL) {
        return;
    }
    frame_t *fb = NULL;
    if (filled_fb_queue) {
        while (xQueueReceive(filled_fb_queue, &fb, 0) == pdPASS) {
            if (fb) {
                // jpeg_alloc_decoder_mem buffers are not freed individually on P4 path;
                // process exit / next allocate recreates pools. Drop struct only.
                free(fb);
            }
        }
    ...
    if (empty_fb_queue) {
        while (xQueueReceive(empty_fb_queue, &fb, 0) == pdPASS) {
            if (fb) {
                free(fb);
            }
        }
