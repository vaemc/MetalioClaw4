...
Metalio Claw4 以 **专业蓝牙音频芯片** 替代 ES8311 + NS4168 分立方案，一套硬件覆盖三种用法：**日常小智对话**（模式 1）、**连接蓝牙耳机/音箱对话**（模式 2）、**手机连本机当音箱**（模式 3）。模式切换、操作步骤与 AT 指令详见 [§12.1 蓝牙音频与三种模式](#121-蓝牙音频与三种模式)。
...
| **触控** | GT911 电容触控（I2C） |
| **音频** | 专业蓝牙音频芯片（替代 ES8311 + NS4168）；I2S 麦克风/扬声器 16 kHz；兼作 Codec / 蓝牙音箱 / 蓝牙耳机连接 |
